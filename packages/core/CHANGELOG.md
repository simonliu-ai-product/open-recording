...
- [`45759b7`](https://github.com/simonliu-ai-product/open-recording/commit/45759b755e203aefaa3120cd3559a1aff77258fc) - Write Chinese transcripts in the script the room actually uses.
...
- [#1](https://github.com/simonliu-ai-product/open-recording/pull/1) [`f4001c1`](https://github.com/simonliu-ai-product/open-recording/commit/f4001c15a766f29559cacf3c3da00e9cfbce947f) Thanks [@LiuYuWei](https://github.com/LiuYuWei)! - Fix a missing `@types/node` in the demo workspace and an accessibility complaint on the screen-recording player, both found by putting the checks in CI.

- [#11](https://github.com/simonliu-ai-product/open-recording/pull/11) [`5198c7d`](https://github.com/simonliu-ai-product/open-recording/commit/5198c7d3fc5d92f73bc1d8e220f12a8a31ee55b7) Thanks [@LiuYuWei](https://github.com/LiuYuWei)! - Describe `transcribe.model` as it behaves. The published types said the path had to be absolute and that a discovered model was the first one found; a relative path is resolved against the workspace, and when several models are present the largest wins.

- [`107be18`](https://github.com/simonliu-ai-product/open-recording/commit/107be18af37884226a9d08bd134d14eef0611526) - The record button now carries the source, and the navigation stops pretending three filters are three pages.
  
  Choosing what to record lived in the bottom-left corner while recording lived in the middle of the page, so recording a tab meant crossing the window between two halves of one action — and the corner's "Arm microphone" was the only brand-coloured button in the sidebar, reading as the app's main action while competing with the real one.
  
  Pressing record now takes a source if none is held and starts, all in one place. A caret beside it picks between the microphone and a browser tab, remembers the choice, and takes it there and then, because a permission prompt and Chrome's tab picker both need the click. A line under the button says what is in hand and how many studio tabs are open.
  
  `Recordings`, `Transcribed`, and `Pending` rendered the same cards over the same recordings. They are one entry now, with the filter beside the search and sort where it belongs.

- [#10](https://github.com/simonliu-ai-product/open-recording/pull/10) [`9503bb1`](https://github.com/simonliu-ai-product/open-recording/commit/9503bb17e8df57c20b4d7f526c11f8292f105cdc) Thanks [@LiuYuWei](https://github.com/LiuYuWei)! - A finished recording can be scrubbed.
...
- [`80edb81`](https://github.com/simonliu-ai-product/open-recording/commit/80edb813a93ba90295d67546d1611eb38dfe0c04) - Give a recording to the tab that is actually holding a microphone.
  ...
  A recording's length is also measured by decoding the file when the container has no duration in it, which is always the case for MediaRecorder output — a real recording no longer reads as `00:00`.

- [`85e74b3`](https://github.com/simonliu-ai-product/open-recording/commit/85e74b3a23b12c2234c2908a6cb41ab704c28410) - See recordings as a table as well as cards.
  
  Cards are for recognising one recording; a table is for comparing many — length, size, tags, and whether each one has a transcript, read down a column instead of across a page. The choice is remembered.

- [`8eabcc7`](https://github.com/simonliu-ai-product/open-recording/commit/8eabcc79d98771855a58a29cc88ed91ebb666c70) - Tag a recording from the page, and sort the table by clicking a column.
